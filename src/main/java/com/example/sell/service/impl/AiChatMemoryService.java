...
import com.example.sell.ai.search.MemoryContextAssembler;
...
public class AiChatMemoryService {
    ...
    /** 记忆上下文最大字符数 */
    private static final int MAX_MEMORY_CONTEXT_CHARS = 8000;
     ...
     * 4. 格式化为文本 "用户: ...\n助手: ...\n"
     * 5. 估算短期上下文 Token，超限则调用小模型压缩
     * 6. 按当前问题从 ES 召回相关历史记忆并拼接
     *
    ...
    /**
     * 加载对话上下文，并基于当前问题从 ES 召回相关历史记忆。
     *
     * @param sessionId 会话ID
     * @param query 当前用户问题
     * @return 组装后的记忆上下文
     */
    public String loadMemoryContext(String sessionId, String query) {
        ...
        try {
            ...
            if (StringUtils.hasText(cachedSummary)) {
                ...
                List<AiSearchCandidate> keywordMemories = loadKeywordMemories(sessionId, query);
                return memoryContextAssembler.assemble(
                        cachedSummary, keywordMemories, MAX_MEMORY_CONTEXT_CHARS);
            }
            ...
            // 3. 短期对话 Token 估算，超限则只压缩短期对话并缓存
            String shortTermContext = messagesText;
            int shortTermTokens = estimateTokens(shortTermContext);
            log.info("[AI记忆] 短期上下文 Token 估算: {}, sessionId={}", shortTermTokens, sessionId);
            if (shortTermTokens > MAX_TOKENS) {
                log.info("[AI记忆] 短期上下文 Token 超限({}>{})，执行压缩, sessionId={}",
                        shortTermTokens, MAX_TOKENS, sessionId);
                shortTermContext = compressContext(shortTermContext);
                cacheSummary(sessionId, shortTermContext);
            }

            // 4. 从 ES 召回与当前问题相关的历史记忆。该部分按问题动态生成，不写入摘要缓存。
            List<AiSearchCandidate> keywordMemories = loadKeywordMemories(sessionId, query);
            String assembled = memoryContextAssembler.assemble(
                    shortTermContext, keywordMemories, MAX_MEMORY_CONTEXT_CHARS);
            if (!StringUtils.hasText(assembled)) {
                return "";
            }

            log.info("[AI记忆] 组装后上下文长度={}, sessionId={}", assembled.length(), sessionId);
            return assembled;
        } catch (Exception e) {
    ...
    private List<AiSearchCandidate> loadKeywordMemories(String sessionId, String query) {
        if (!StringUtils.hasText(query)) {
            return Collections.emptyList();
        }
        try {
            AiRequestContextHolder.AiRequestContext ctx = AiRequestContextHolder.get();
            String userId = ctx == null ? null : ctx.userId();
            Long windowStartMs = ctx == null ? null : ctx.windowStart().toEpochMilli();
            List<AiSearchCandidate> memories = elasticAiSearchService.searchMemory(
                    query, userId, sessionId, windowStartMs, 6);
            log.info("[AI记忆] ES 相关历史记忆召回 {} 条, sessionId={}", memories.size(), sessionId);
            return memories;
        } catch (Exception e) {
            log.warn("[AI记忆] ES 相关历史记忆召回失败, sessionId={}", sessionId, e);
            return Collections.emptyList();
        }
    }
